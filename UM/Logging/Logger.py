...
class Logger:
    ...
    @classmethod
    def addLogger(cls, logger: "LogOutput"):
        cls.__loggers.append(logger)

    ##  Get all loggers
    #   \returns \type{list} List of Loggers
    @classmethod
    def getLoggers(cls) -> List["LogOutput"]:
...
class LogOutput:
    ...
    def log(self, log_type: str, message: str) -> None:
        if log_type.lower().startswith("w"):
            self._logger.warning(message)
        elif log_type.lower().startswith("i"):
            self._logger.info(message)
        elif log_type.lower().startswith("e"):
            self._logger.error(message)
        elif log_type.lower().startswith("d"):
            self._logger.debug(message)
        elif log_type.lower().startswith("c"):

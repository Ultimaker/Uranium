...
class LocalContainerProvider(ContainerProvider):
    ...
    def __init__(self, container_registry):
        super().__init__(container_registry)

        self._id_to_path = {}  # type: Dict[str, str]
        self._id_to_mime = {}  # type: Dict[str, MimeType]
    ...
    def loadContainer(self, container_id: str) -> "ContainerInterface":
        ...
        file_path = self._id_to_path[container_id]
        ...
        container_class = self._container_registry.mime_type_map[self._id_to_mime[base_id].name]
        ...
        return self._container_registry.findContainers(id = container_id)[0]
    ...
    def saveContainer(self, container: "ContainerInterface") -> None:
        ...
        mime_type = self._container_registry.getMimeTypeForContainer(type(container))
        file_name = urllib.parse.quote_plus(container.getId()) + "." + mime_type.preferredSuffix
        container_type = container.getMetaDataEntry("type")
        resource_types = self._container_registry.getResourceTypes()
        if container_type in resource_types:
            ...
            if base_file:
                for container_md in self._container_registry.findContainersMetadata(base_file = base_file):
    ...
    def loadMetadata(self, container_id: str) -> Optional[Dict[str, Any]]:
        if container_id in self._container_registry.metadata:
            return self._container_registry.metadata[container_id]

        filename = self._id_to_path[container_id]
        clazz = self._container_registry.mime_type_map[self._id_to_mime[container_id].name]
        ...
        for metadata in result_metadatas:
            ...
            if metadata["id"] not in self._container_registry.metadata:  # This wouldn't get loaded normally.
                ...
                self._container_registry.metadata[metadata["id"]] = metadata
                self._container_registry.source_provider[metadata["id"]] = self
    ...
    #   ``None``.
    def _loadCachedDefinition(self, definition_id: str) -> Optional[DefinitionContainer]:
        definition_path = self._id_to_path[definition_id]
        try:
            cache_path = os.path.join(self._container_registry.definition_cache_dir, definition_id)
        ...
        try:
            ...
            Logger.log("w", "Failed to load definition {definition_id} from cached file: {error_msg}".format(
                definition_id = definition_id, error_msg = str(e)))
            return None
    ...
    def _saveCachedDefinition(self, definition: DefinitionContainer) -> None:
        cache_path = os.path.join(self._container_registry.definition_cache_dir, definition.getId())
    ...
    def _updatePathCache(self) -> None:
        ...
        all_resources = set()
        for resource_type in self._container_registry.getResourceTypes().values():
            all_resources |= set(Resources.getAllResourcesOfType(resource_type))
    ...
    def _pathToMime(self, path: str) -> Optional[str]:
        mime_type = None
        try:
            mime_type= MimeTypeDatabase.getMimeTypeForFile(path)
        except MimeTypeNotFoundError:
            Logger.log("w", "MIME type could not be found for file: {path}, ignoring it.".format(path = path))
            return None

        # The MIME type is known, but it's not a container.
        if mime_type.name not in self._container_registry.mime_type_map:
            mime_type = None
